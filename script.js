...
function formatDateLocal(d) {
  const dt = new Date(d);
  if (isNaN(dt)) return "-";
  return dt.toLocaleDateString(currentLang === "bn" ? "bn-BD" : "en-US");
}
function uid() {
...
function translateUI() {
  if (currentLang === "en") {
    ...
    $("appSubtitle").textContent =
      "টিকা শিডিউল সংরক্ষণ করুন, রিমাইন্ডার পেয়ে যান";
...
function renderBabies() {
  ...
  state.babies.forEach((b) => {
    ...
    el.className = "baby-item";
    el.innerHTML = `<div><strong>${escapeHtml(
      b.name
    )}</strong><div class="small" style="color:var(--muted)">DOB: ${
      b.dob ? formatDateLocal(b.dob) : "-"
    } ${
      b.gender
        ? " • " +
          (b.gender === "male"
            ...
            : "Female")
        : ""
    }</div></div><div style="display:flex;gap:8px"><button class="btn ghost" onclick="selectBaby('${
      b.id
    }')">${
      state.selectedBabyId === b.id
        ...
        ? "বেছে নিন"
        : "Select"
    }</button><button class="btn" onclick="deleteBaby('${b.id}')">${
...
function getVaccineListForBaby(baby) {
  const template =
    defaultTemplates[state.template] || defaultTemplates.standard;
  return [...template, ...state.customVaccines].map((v) => ({
    name: v.name,
    weeks: v.weeks,
...
function renderVaccines() {
  const section = $("vaccineList");
  section.innerHTML = "";
  const baby = state.babies.find((b) => b.id === state.selectedBabyId);
  if (!baby) {
    section.innerHTML = `<div class="small" style="color:var(--muted)">${
      currentLang === "bn"
        ? "দয়া করে বাম থেকে একজন শিশু যোগ করে নির্বাচন করুন।"
        : "Please add and select a baby from left."
    }</div>`;
    return;
  }
  const q = $("searchInput").value.toLowerCase();
  const filter = $("filterSelect").value;
  const vaccines = getVaccineListForBaby(baby).sort(
    (a, b) => new Date(a.due) - new Date(b.due)
  );
  vaccines.forEach((v) => {
    const today = new Date();
    const dueDate = new Date(v.due);
    const doneKey = `done_${baby.id}_${v.name}_${dueDate
      .toISOString()
      .slice(0, 10)}`;
    const done = localStorage.getItem(doneKey) === "1";
    const isOverdue =
      dueDate < new Date(new Date().setHours(0, 0, 0, 0)) && !done;
    const isDueSoon =
      dueDate - today <= 7 * 24 * 60 * 60 * 1000 && dueDate - today >= 0;
    const matchesQ =
      v.name.toLowerCase().includes(q) || baby.name.toLowerCase().includes(q);
    if (!matchesQ) return;
    if (filter === "completed" && !done) return;
    if (filter === "pending" && done) return;
    if (filter === "overdue" && !isOverdue) return;
    const el = document.createElement("div");
    el.className = "v-item";
    if (done) el.classList.add("done");
    el.innerHTML = `<div><div style="font-weight:600">${escapeHtml(
      v.name
    )}</div><div class="small" style="color:var(--muted)">${
      currentLang === "bn" ? "নির্ধারিত" : "Due"
    }: ${formatDateLocal(dueDate)} • ${v.weeks} ${
      currentLang === "bn" ? "সপ্তাহ পরে" : "weeks after birth"
    }</div></div><div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end"><div style="font-size:.9rem;color:${
      isOverdue ? "var(--danger)" : isDueSoon ? "crimson" : "var(--muted)"
    }">${
      isOverdue
        ? currentLang === "bn"
          ? "Overdue"
        ...
        ? "পরবর্তী"
        : "Upcoming"
    }</div><div style="display:flex;gap:8px"><button class="btn ghost" onclick="toggleDone('${doneKey}')">${
      done
        ? currentLang === "bn"
          ? "অনমার্ক"
          : "Unmark"
        ...
        ? "মার্ক সম্পন্ন"
        : "Mark done"
    }</button><button class="btn" onclick="snooze('${baby.id}','${escapeJs(
      v.name
    )}','${dueDate.toISOString().slice(0, 10)}')">${
      currentLang === "bn" ? "স্মরণ" : "Snooze"
    }</button></div></div>`;
    if (isOverdue)
      el.querySelector(".small").insertAdjacentHTML(
        "beforebegin",
        `<div class="badge overdue">${
          currentLang === "bn" ? "ওভারডিউ" : "Overdue"
        }</div>`
      );
    ...
    if (
      ...
      notify(`${baby.name} - ${v.name}`, `${formatDateLocal(dueDate)}`);
    }
  });
}
...
function renderProgress() {
  ...
  if (!baby) {
    ...
    $("progressText").textContent =
      currentLang === "bn" ? "0% completed" : "0% completed";
    return;
  }
  const vacs = getVaccineListForBaby(baby);
  const total = vacs.length;
  let done = 0;
  vacs.forEach((v) => {
    const dueDate = new Date(v.due);
    const key = `done_${baby.id}_${v.name}_${dueDate
      .toISOString()
      .slice(0, 10)}`;
...
function renderCalendar() {
  ...
  $("calendarMonth").textContent = start.toLocaleString(
  ...
  for (let d = 1; d <= days; d++) {
    ...
    state.babies.forEach((b) => {
      getVaccineListForBaby(b).forEach((v) => {
        const due = new Date(v.due);
        if (due.toISOString().slice(0, 10) === iso) {
          const doneKey = `done_${b.id}_${v.name}_${iso}`;
          const done = localStorage.getItem(doneKey) === "1";
          const el = document.createElement("div");
          el.innerHTML = `<div class="small">${escapeHtml(
            b.name
          )} - ${escapeHtml(v.name)} ${done ? "✓" : ""}</div>`;
          if (!done && due < new Date(new Date().setHours(0, 0, 0, 0)))
...
function deleteBaby(id) {
  if (!confirm(currentLang === "bn" ? "মুছে ফেলতে চান?" : "Delete?")) return;
  state.babies = state.babies.filter((b) => b.id !== id);
  if (state.selectedBabyId === id) state.selectedBabyId = null;
  save();
  renderAll();
}
function addBaby() {
  const name = $("babyName").value.trim();
  const dob = $("babyDob").value;
  const gender = $("babyGender").value;
  if (!name || !dob) {
    alert(
      currentLang === "bn"
        ? "নাম এবং জন্মতারিখ দিন"
        : "Please provide name and DOB"
    );
    return;
  }
  const newB = { id: uid(), name, dob, gender };
  state.babies.push(newB);
  state.selectedBabyId = newB.id;
  save();
  $("babyName").value = "";
  $("babyDob").value = "";
  renderAll();
...
function clearAll() {
  if (
    !confirm(
      currentLang === "bn"
        ? "সব তথ্য মুছে ফেলা হবে — আপনি কি নিশ্চিত?"
        : "All data will be cleared — are you sure?"
    )
  )
    return;
...
function addCustomVaccine() {
  const name = $("customVaccineName").value.trim();
  const weeks = $("customVaccineWeeks").value;
  if (!name || weeks === "") {
    alert(
      currentLang === "bn"
        ? "টিকা নাম ও সপ্তাহ দিন"
        : "Provide vaccine name and weeks"
    );
    return;
  }
  state.customVaccines.push({ name, weeks });
  save();
  $("customVaccineName").value = "";
  $("customVaccineWeeks").value = "";
  renderAll();
}
...
function snooze(babyId, vname, vdate) {
  const t = prompt(
    currentLang === "bn"
      ? "কত মিনিট পরে স্মরণ করবেন?"
      : "Remind after how many minutes?",
    "60"
  );
  const min = Number(t);
  if (!min || min <= 0) return;
  setTimeout(() => {
    notify(
      vname + " — " + vdate,
      currentLang === "bn"
        ? "সময় হয়েছে — টিকা দেয়া প্রয়োজন।"
        : "Time to vaccinate"
    );
    alert(
      (currentLang === "bn" ? "স্মরণ: " : "Reminder: ") + vname + " - " + vdate
    );
  }, min * 60 * 1000);
  alert(
    currentLang === "bn"
      ? "স্মরণ নির্ধারিত — অ্যাপ খুলে থাকলে কাজ করবে।"
      : "Snooze set — will work if app is open."
  );
}
...
function requestNotification() {
  if (!("Notification" in window)) {
    alert(
      currentLang === "bn"
        ? "আপনার ব্রাউজারে নোটিফিকেশন সাপোর্ট নেই"
  ...
  Notification.requestPermission().then((p) => {
    if (p === "granted")
      alert(
        currentLang === "bn"
          ? "নোটিফিকেশন অনুমোদন দেওয়া হয়েছে"
          : "Notifications granted"
      );
    else alert(currentLang === "bn" ? "অনুমোদন দেওয়া হয়নি" : "Not granted");
  });
}
...
function exportICal() {
  ...
  if (!baby) {
    alert(
      currentLang === "bn"
        ? "প্রথমে একটি শিশু নির্বাচন করুন"
        : "Select a baby first"
    );
    return;
  }
  const events = getVaccineListForBaby(baby).map((v) => {
    const start = new Date(v.due);
    const end = new Date(start.getTime() + 30 * 60 * 1000);
    return {
      title: `${baby.name} - ${v.name}`,
      start,
      end,
      desc: "Vaccine reminder",
    };
  });
  ...
  a.href = url;
  a.download = `${baby.name.replace(/\\s+/g, "_")}_vaccines.ics`;
  a.click();
    ...
    tokenClient = google.accounts.oauth2.initTokenClient({
      ...
      callback: (resp) => {
        if (resp.error) {
          ...
          alert("Authenticated — you can now sync to Google Calendar");
        }
      },
    });
  } catch (err) {
    console.error("gapi init failed", err);
    alert("Google API init failed: " + (err.message || err));
  }
}
...
function connectAndSyncGoogle() {
  if (!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID.includes("REPLACE_WITH")) {
    alert(
      "Please set GOOGLE_CLIENT_ID in the code (create OAuth credentials in Google Cloud Console)"
    );
    return;
  }
  if (!gapiInited) {
    gapiLoad();
    setTimeout(() => {
      connectAndSyncGoogle();
    }, 800);
    return;
  }
  tokenClient.requestAccessToken({ prompt: "consent" });
  setTimeout(() => {
    syncSelectedBabyToGoogle();
  }, 1400);
}
...
async function syncSelectedBabyToGoogle() {
  ...
  alert(
    (currentLang === "bn" ? "সম্পন্ন: " : "Done: ") +
      ...
      events.length +
      (currentLang === "bn"
        ? " ইভেন্ট Google Calendar-এ যোগ করা হয়েছে।"
